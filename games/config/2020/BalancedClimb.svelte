...
let dockButton = "";
if ($gameState === 0) {
    dockButton = "AutoBalanced";
} else {
    dockButton = "TeleBalanced";
}
...
<button class="btn {$gameData[dockButton] === 0
    ...
    ? "btn-secondary"
    : "btn-success"} {$gameData["Park"] === 1 ? "btn-disabled" : "" || $gameData["TeleHanging"] === 2 ? "btn-disabled" : ""}" on:click={handleClick}>Balanced?</button>

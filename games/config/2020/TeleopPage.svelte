<script>
  import Nodes from "./Nodes.svelte";
  import LocationSelector from "./LocationSelector.svelte";
  import TrashCan from "./TrashCan.svelte";
  import Undo from "./Undo.svelte";
  import Parked from "./Parked.svelte";
  import { teleDataLog } from "./stores";
  import CurrentTeam from "./CurrentTeam.svelte";
  import ActiveGamePiece from "./ActiveGamePiece.svelte";
  import { liveGamepiece } from "./stores";
</script>

<!-- <ScoringGrid /> -->
<div class="absolute left-[450px] top-[80px]">
  <Nodes level="4" type="Success" gameMode="Tele" />
</div>

<div class="absolute left-[580px] top-[80px]">
  <Nodes level="4" type="Fail" gameMode="Tele" />
</div>
<div class="absolute left-[130px] top-[40px]">
  <Nodes level="3" type="Success" gameMode="Tele" />
</div>
<!-- <div class="absolute left-[200px] top-[140px]">
  <Nodes level="3" type="Fail" gameMode="Tele" />
</div> -->
<div class="absolute left-[60px] top-[140px]">
  <Nodes level="2" type="Success" gameMode="Tele" />
</div>
<div class="absolute left-[200px] top-[140px]">
  <Nodes level="2" type="Fail" gameMode="Tele" />
</div>
<div class="absolute left-[60px] top-[250px]">
  <Nodes level="1" type="Success" gameMode="Tele" />
</div>
<div class="absolute left-[200px] top-[250px]">
  <Nodes level="1" type="Fail" gameMode="Tele" />
</div>
<div class="absolute left-[450px] top-[215px]">
  <Nodes level="5" type="Success" gameMode="Tele" />
</div>

<div class="absolute left-[580px] top-[215px]">
  <Nodes level="5" type="Fail" gameMode="Tele" />
</div>

<!-- <div class="absolute left-[425px] top-[350px]">
  <Nodes level="4" type="Success" gameMode="Tele" />
</div> -->

<div class="absolute left-[500px]">
  <LocationSelector gameMode="Tele" />
</div>

<!-- <div class="absolute ml-[525px] mt-[30px]">
  <div class="flex flex-col gap-y-2">
    <Parked />
  </div>
</div>
<div class="absolute ml-[675px] mt-[30px]">
  <div class="flex flex-col gap-y-2">
    <OnStage />
  </div>
</div> -->

<div>
  <div class="flex flex-row gap-x-[50px] absolute left-[750px] top-[375px]">
    <Undo />
  </div>
</div>

<div>
  {#if $liveGamepiece[0] == "Cells"}
  <div>
    <div class="absolute ml-[750px] mt-[75px]">
      
      <ActiveGamePiece activeGamePiece = "Cells" />
    
    </div>
  </div>
  {/if}
  {#if $liveGamepiece[1] == "Cells"}
  <div>
    <div class="absolute ml-[750px] mt-[125px]">
      
      <ActiveGamePiece activeGamePiece = "Cells" />
    
    </div>
  </div>
  {/if}
  {#if $liveGamepiece[2] == "Cells"}
  <div>
    <div class="absolute ml-[750px] mt-[175px]">
      
      <ActiveGamePiece activeGamePiece = "Cells" />
    
    </div>
  </div>
  {/if}
  {#if $liveGamepiece[3] == "Cells"}
  <div>
    <div class="absolute ml-[700px] mt-[75px]">
      
      <ActiveGamePiece activeGamePiece = "Cells" />
    
    </div>
  </div>
  {/if}
  {#if $liveGamepiece[4] == "Cells"}
  <div>
    <div class="absolute ml-[700px] mt-[125px]">
      
      <ActiveGamePiece activeGamePiece = "Cells" />
    
    </div>
  </div>
  {/if}
</div>

<div class="flex flex-row gap-x-[50px] absolute left-[425px] top-[350px]">
  <div>
    <TrashCan gameMode="Tele" />
  </div>
</div>

<div class="ml-[775px] mt-[30px]">
  <CurrentTeam />
</div>

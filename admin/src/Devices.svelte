    ...
    <tbody class="rounded-md">
        {#each devices as device}
            <tr>
                <td>{device.name}</td>
                <td
                    class={Math.round(Date.now() / 1000) - device.last_heartbeat > 30
                        ? "bg-red-500"
                        : Math.round(Date.now() / 1000) - device.last_heartbeat < 10 ? "bg-green-500" : "bg-yellow-500" }>{formattedTime(device.last_heartbeat)}</td
                >

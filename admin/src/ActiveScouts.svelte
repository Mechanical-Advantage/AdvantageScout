<script>
    ...
    async function getScouts() {
        const response = await fetch("/get_scouts", { method: "GET" });
    ...
    function getDropBoxHeight() {
        if (enabled.length > 8) {
            return (dropBoxHeight =
                200 + Math.ceil((enabled.length - 8) / 4) * 65);
        } else {
...
>>>>>>> 12e3307 (work on match and block schedule modals)
    ...
    function deleteName(event) {
        disabled = disabled.filter((e) => e !== event.detail.text);
        enabled = enabled.filter((e) => e !== event.detail.text);
        doPost(event.detail.text, "/remove_scout");
        enabled = enabled;
    }

    function addName() {
        if (
            !disabled.includes(addScoutEntry) &&
...
<div
    class="z-20 w-fit h-fit"
    on:mousemove={mouseCoordHandler}
    on:mouseup={mouseUp}
>
    <label
        class="btn modal-button btn-success btn-square w-24 h-24 absolute ml-[630px]"
        for="entry-modal"
    >
        <svg
    ...
    <div
        id="dropBox"
        style="width:600px;height:{dropBoxHeight}px;border:4px solid {!(
            $movin &&
            $withinBox &&
            !$activeState
        )
